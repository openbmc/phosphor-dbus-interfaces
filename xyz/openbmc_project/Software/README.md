...
The build process results in creation of the various files which together
...
they are packaged. As part of packaging, calculate “digest” by executing
hash function on the image and use the private key to sign the digest.
...
2. Extract the manifest file and do the validation of the the Manifest
...
4. Validate the files(except Manifest file) using BMC public key and hash
   function provided by manifest file.
     -For any failure during this process report failure and stop the process.
5. After Successful validation continue the normal code update process.
...
2. Packaging, should include Hash function, key and type(Witherspoon,public key)
   information as part of manifest file.
...
5. Since the un-signed flash image doesn’t have a business logic to verify
   the signed image, un-signed to signed image code upgrade works smoothly.
