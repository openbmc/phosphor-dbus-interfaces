...
examples, but the interface is not specific to a device type. It tracks the
current state (`CurrentDeviceState`) and the requested transition
(`RequestedDeviceTransition`) in the same way the primary states do.
...
owns it is off, the device reports (NotReady) and uses that error to reject a
